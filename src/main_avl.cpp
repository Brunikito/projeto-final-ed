...
void printUsage() {
    std::cerr << "Uso: ./avl <search\\stats> <n_docs> <diretorio>" << std::endl;
...
int main(int argc, char* argv[]){
	...
	TreeOperations avl_ops = TreeOperations{AVL::create, AVL::destroy, AVL::insert, AVL::search};

    // Decida qual função chamar com base no comando
    if (command == "search") {
        searchTree(tree, avl_ops, "AVL", command, n_docs, filesDir);
        return 0;
    }
    runStats(tree, avl_ops, "AVL", command, n_docs, filesDir);
	return 0;
}