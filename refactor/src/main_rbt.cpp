...
void printUsage() {
    std::cerr << "Uso: ./rbt <search\\stats> <n_docs> <diretorio>" << std::endl;
...
int main(int argc, char* argv[]){
	...
	std::cout << "Criando arvore RBT..." << std::endl;
    BinaryTree* tree = RBT::create();
    if (!tree) {
        std::cerr << "Erro: Falha ao criar a arvore RBT." << std::endl;
        return 1;
    }

	TreeOperations rbt_ops = TreeOperations{RBT::create, RBT::destroy, RBT::insert, RBT::search};

    // Decida qual função chamar com base no comando
    if (command == "search") {
        searchTree(tree, rbt_ops, "RBT", command, n_docs, filesDir);
        return 0;
    }
    runStats(tree, rbt_ops, "RBT", command, n_docs, filesDir);
	return 0;
}